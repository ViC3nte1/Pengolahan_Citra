...
print("Total MCQ Object Created:", len(mcqList))

qNo = 0
qTotal = len(dataAll)

cv2.namedWindow("Img", cv2.WINDOW_NORMAL)
cv2.resizeWindow("Img", 1536, 864)

while True:
    success, img = cap.read()
    img = cv2.flip(img, 1)  # Flip the image horizontally
    hands, img = detector.findHands(img, flipType=False)  # returns both image and hand info
